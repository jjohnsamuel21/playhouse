...
export default function RankThemeSelect() {
  ...
  useEffect(() => {
    if (!user) return;
    const publicQ = query(collection(db, 'themes'), where('gameId', '==', 'ranking'), where('visibility', '==', 'public'));
    const ownedQ = query(collection(db, 'themes'), where('gameId', '==', 'ranking'), where('ownerId', '==', user.uid));
