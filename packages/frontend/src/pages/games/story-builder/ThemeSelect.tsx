...
const visual = GAME_VISUALS['story-builder'];

export default function StoryThemeSelect() {
  ...
  useEffect(() => {
    if (!user) return;
    const publicQ = query(collection(db, 'themes'), where('gameId', '==', 'story-builder'), where('visibility', '==', 'public'));
    const ownedQ = query(collection(db, 'themes'), where('gameId', '==', 'story-builder'), where('ownerId', '==', user.uid));
    ...
    <div className="page-layer min-h-screen p-6 animate-fadeUp">
      <div className="max-w-lg mx-auto">
        ...
        <div className="flex items-center gap-3 mt-4 mb-6">
          ...
          <div>
            <h1 className="font-display font-bold text-2xl text-playhouse-text-primary m-0">Story Builder</h1>
            <p className="text-playhouse-text-secondary text-[13.5px] mt-0.5 m-0">Pick a story starter</p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {themes.map((theme) => (
              <button
                key={theme.id}
                onClick={() => navigate(`/game/story-builder/local-setup/${theme.id}`)}
