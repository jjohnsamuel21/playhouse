...
const visual = GAME_VISUALS['trivia'];

export default function TriviaThemeSelect() {
  ...
  useEffect(() => {
    if (!user) return;
    const publicQ = query(collection(db, 'themes'), where('gameId', '==', 'trivia'), where('visibility', '==', 'public'));
    const ownedQ = query(collection(db, 'themes'), where('gameId', '==', 'trivia'), where('ownerId', '==', user.uid));
    ...
    <div className="page-layer min-h-screen p-6 animate-fadeUp">
      <div className="max-w-lg mx-auto">
        ...
        <div className="flex items-center gap-3 mt-4 mb-6">
          ...
          <div>
            <h1 className="font-display font-bold text-2xl text-playhouse-text-primary m-0">Trivia Night</h1>
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {themes.map((theme) => (
              <button
                key={theme.id}
                onClick={() => navigate(`/game/trivia/local-setup/${theme.id}`)}
